            ...
            <div class="nav-search" id="nav-search">
                <form action="" method="get" class="form-search">
                    ...
                    <select name="type" id="form-field-1" class="searchtype">
                        ...
                        <option value="2" @if($data->type == 2) selected @endif >章节标题</option>
                        ...
                        <div class="table-responsive">
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                @foreach($data as $notes)
                                    ...
                                    <tr>
                                        <td>{{$notes->id}}</td>
                                        <td>{{$notes->parchaptertitle}}</td>
                                        <td class="commentContentjs">{{$notes->notecontent}}</td>
                                        <td>{{$notes->username}}</td>
                                        <td>{{$notes->coursetype ? '文档课程' : '视频课程'}}</td>
                                        <td>{{$notes->public ? '私密' : '公开'}}</td>
                                        <td>{{$notes->createtime}}</td>
                                        <td>
                                            <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">

                                                <a href="{{url('/admin/specialCourse/detailNotes/'.$notes->id)}}"  class="btn btn-xs btn-success">
                                                    <i class="icon-ok bigger-80">查看</i>
                                                </a>

                                                @permission('del.course')
                                                <a href="{{url('/admin/specialCourse/delNotes/'.$notes->id)}}" style="width:29px" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
