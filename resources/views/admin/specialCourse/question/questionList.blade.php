            ...
            <div class="nav-search" id="nav-search">
                <form action="" method="get" class="form-search">
                    ...
                    <select name="type" id="form-field-1" class="searchtype">
                        ...
                        <option value="2" @if($data->type == 2) selected @endif>提问内容</option>
                        <option value="3" @if($data->type == 3) selected @endif>提问用户</option>
                        ...
                        <div class="table-responsive">
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                @foreach($data as $d)
                                    ...
                                    <tr>
                                        <td>{{$d->id}}</td>
                                        <td class="commentContentjs">{{$d->content}}</td>
                                        <td>{{$d->username}}</td>
                                        <td>{{$d->asktime}}</td>
                                        <td>
                                            @if($d->status == 1)
                                        ...
                                        <td>
                                            <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                ...
                                                <a href="{{url('/admin/specialCourse/detailQuestion/'.$d->id)}}" class="btn btn-xs btn-info">
                                                    查看提问及回答详情
                                                </a>
                                                ...
                                                <a href="{{url('/admin/specialCourse/delQuestion/'.$d->id)}}" style="width:29px" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
