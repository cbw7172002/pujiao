    ...
    <div class="main-content">
        <div class="breadcrumbs" id="breadcrumbs">
            ...
            <ul class="breadcrumb">
                ...
                <li>
                    <a href="{{url('/admin/commentReply/questionCommentList')}}">评论回复管理</a>
                </li>
                <li class="active">问答评论列表</li>
            </ul><!-- .breadcrumb -->
                        ...
                        <div class="table-responsive">
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                @foreach($data as $d)
                                    ...
                                    <tr>
                                        <td>{{$d->id}}</td>
                                        <td class="commentContentjs">{{$d->commentContent}}</td>
                                        <td>{{$d->qestitle}}</td>
                                        <td>{{$d->parentId}}</td>
                                        <td>{{$d->username}}</td>
                                        <td>{{$d->tousername}}</td>
                                        ...
                                        <td>
                                            <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                ...
                                                <a href="{{url('/admin/commentReply/editquestionComment/'.$d->id)}}" class="btn btn-xs btn-info">
                                                    <i class="icon-edit bigger-120"></i>
                                                </a>
                                                @endpermission

                                                @permission('delete.commentReply')
                                                <a href="{{url('/admin/commentReply/delquestionComment/'.$d->id)}}" style="width:29px" class="btn btn-xs btn-danger" onclick="return confirm('删除后将无法找回,确定要删除吗?');">
                                                ...
                                                <a href="{{url('/admin/commentReply/lookquestionComment/'.$d->id)}}"  class="btn btn-xs btn-success">
