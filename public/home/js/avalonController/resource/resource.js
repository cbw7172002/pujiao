define([], function () {
    ...
    var vm = avalon.define({
        ...
        resources:[],         //资源结果集
        selTitle:'',          //搜索title
        selGradeId:0,sideSelGradeId:0,        //年级
        selSubjectId:0,sideSelSubjectId:0,    //科目
        selEditionId:0,sideSelEditionId:0,    //版本
        selBookId:0,sideSelBookId:0,          //册别
        selChapterId:0,sideSelChapterId:0,    //章节
        selTypeId:0,          //资源类型
        orderBy:'resourceView', //created_at
        isExpand:false,sideIsExpand:false,
        isSideClick:false,    //侧边栏点击
        gradesShowNum:8,      //年级选项个数配置
        subjectsShowNum:7,    //科目选项个数配置  
        editionsShowNum:8,    //版本选项个数配置  
        booksShowNum:8,       //册别选项个数配置
        //获取选择数据
        ...
        // 年级-科目-版本-册别 选择
        selecting:function(type,gradeId,subjectId,editionId,bookId,chapter,isSide){      //获取资源
            vm.isSideClick = isSide || false;
            //console.log('左侧点击？ '+vm.isSideClick);
            if(!vm.isSideClick){
                if (type == 1) {                                                         //年级
                    vm.selGradeId = vm.sideSelGradeId = gradeId ;
                }
                if (type == 2) { vm.isExpand = false; vm.selSubjectId = vm.sideSelSubjectId = gradeId }             //科目
                if (type == 3) vm.selEditionId = vm.sideSelEditionId = gradeId;          //版本
                if (type == 4) vm.selBookId = vm.sideSelBookId = gradeId;                //册别
                if (type == 5) { vm.selSubjectId = null; vm.isExpand = true }            //拓展
                if (type == 6) vm.selChapterId = gradeId;                                //章节

            }else{
                vm.sideIsExpand = false;                                                 //默认非拓展
                vm.sideSelGradeId = gradeId;                                             //年级
                vm.sideSelSubjectId = subjectId;                                         //科目
                vm.sideSelEditionId = editionId;                                         //版本
                vm.sideSelBookId = bookId;                                               //册别
                vm.sideSelChapterId = chapter;                                           //章节

                if (type == 5) { vm.sideSelSubjectId = null; vm.sideIsExpand = true }    //拓展
                if (type == 6) vm.sideSelChapterId = chapter;                            //章节
            }
            if (type == 7) vm.selTypeId = gradeId;                                       //类型
            if (type == 8) vm.orderBy = gradeId ? 'resourceView' : 'created_at';         //排序
            vm.getResource();                                                            //获取资源
        },
        ...
        formationData:function(pageNumber,pageSize){
            var isexpand;
            if(vm.isSideClick) isexpand = vm.sideIsExpand;
            else isexpand = vm.isExpand;
            var data =  isexpand ? {
                ...
                where:{
                    isexpand:2,
                    resourceIsDel:0,
                    resourceGrade:vm.isSideClick ? vm.sideSelGradeId : vm.selGradeId,
                },
                whereb:{
                ...
                where:{
                    ...
                    resourceIsDel:0,
                    resourceGrade:vm.isSideClick ? vm.sideSelGradeId : vm.selGradeId,
                    resourceSubject:vm.isSideClick ? vm.sideSelSubjectId : vm.selSubjectId,
                    resourceEdition:vm.isSideClick ? vm.sideSelEditionId : vm.selEditionId,
                    resourceBook:vm.isSideClick ? vm.sideSelBookId : vm.selBookId,
                    resourceChapter:vm.isSideClick ? vm.sideSelChapterId : vm.selChapterId,
        ...
        getResource:function(){
            ...
            //return false;
            $('.clearPageCon').html('<div id="page"></div>');
            $('#page').pagination({
