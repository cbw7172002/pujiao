    ...
    var vm = avalon.define({
        ...
        uploadziliao:function(data,datalength){
            for (var i = 0; i < datalength; i++) {
                var arrlastindex = vm.resData.push(
                    {
                        'resourceTitle':data[i].name,    //资源名称
                        'resourceIntro':'',              //资源介绍
                        'resourcePic':data[i].pic,       //资源封面
                        'resourceFormat':data[i].format, //资源格式
                        'fileID':'',                     //文件ID
                ...
                if(arrlastindex <= vm.fileLimit){
                    vm.fordataupload(data[i],vm.resData[arrlastindex-1],arrlastindex-1);
                }else{
                    alert('上传文件最多不超过'+vm.fileLimit+'个'); return false;
                }
            }
        },
        ...
        commitResData:function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.status){
                        alert('提交成功');
                        window.location.href = '/member/teacherHomePage/'+ vm.userId +'#myResource';
                    }else{
                        alert('提交失败')
                    }
                },
