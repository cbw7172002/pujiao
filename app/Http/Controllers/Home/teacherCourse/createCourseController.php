    ...
    public function addCourse(){
        ...
        if(isset($_POST['prelearnInfo'])){
            foreach ( $_POST['prelearnInfo'] as $a_a){
                try{
                    $a_a['title'] = Filter::filter($a_a['title']);
                }catch (Exception $e){
                    Log::info('添加课程章节信息敏感词过滤报错：'.$e->getMessage());
                }
                if(isset($a_a['id'])){
                    //更新导学title
        ...
        if(isset($_POST['chaprerInfo'])){
            foreach ($_POST['chaprerInfo'] as $b_a){
                try{
                    $b_a['title'] = Filter::filter($b_a['title']);
                }catch (Exception $e){
                    Log::info('添加课程章节信息敏感词过滤报错：'.$e->getMessage());
                }
                if(isset($b_a['id'])){
                    ...
                    foreach ($b_a['nodeInfo'] as $b_b){
                        try{
                            $b_b['title'] = Filter::filter($b_b['title']);
                        }catch (Exception $e){
                            Log::info('添加课程章节信息敏感词过滤报错：'.$e->getMessage());
                        }
                        if(isset($b_b['id'])){
        ...
        if(isset($_POST['guideInfo'])){
            foreach ( $_POST['guideInfo'] as $c_a){
                try{
                    $c_a['title'] = Filter::filter($c_a['title']);
                }catch (Exception $e){
                    Log::info('添加课程章节信息敏感词过滤报错：'.$e->getMessage());
                }
                if(isset($c_a['id'])){
